...
1. **Add a trial-ending placement to your config**
   ...
   The `trial_ending` trigger fires when a trial enters its closing window. This is a complete, valid `Playbook`.
...
2. **Provide trial status to the SDK**
   ...
   The SDK reads trial state from a resolver in local mode (or from the user context in hosted mode). Supply the trial so the trigger and `{{days_remaining}}` token can compute the window.
   ...
   <RevTurbineProvider
     options={{
       ...
       user: { id: 'user_123', plan_handle: 'trial' },
     }}
   >
     <AppHeader />
   </RevTurbineProvider>
