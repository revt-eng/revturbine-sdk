...
description: How RevTurbine splits responsibility — RevTurbine owns monetization policy, your app owns enforcement, placements are additive while entitlement checks fail closed, and billing-critical actions are re-checked on your server. Plus the difference between <Gate>, gate(), and can().
sidebar:
  order: 2
---
...
RevTurbine is an *additive* layer: your product must look and work correctly with every placement paused and the SDK offline. A monetization layer should never be able to take your product *down* — placements render nothing. It should also never let downtime *leak* paid access — so entitlement checks deny when they can't affirm a grant.
</Aside>
...
- [Error Handling](/guides/error-handling/) — additive placements and fail-closed checks in detail
