...
<Aside type="tip">
`npx revturbine create` performs steps 1–2 for you (and also pins the CLI and installs the Agent Skills). Run it, then continue from step 3.
</Aside>
...
2. **Add an exported config**
    ...
    Export your `playbook.json` from the RevTurbine dashboard and place it in your `src/` directory.
