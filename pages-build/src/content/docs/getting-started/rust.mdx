...
field you do not set. Only `tenant_id` and `user_id` are required; construction
rejects either one when it is empty.
...
Import `UserContext` from `revturbine::sdk`, as shown above. Do not substitute
the generated persisted model at `revturbine::types::UserContext`, and do not
copy TypeScript's broader `RevTurbineUserContext` shape. The local evaluator
accepts `plan_handle` plus optional `plan_name` rather than a `plan` object,
and `trial_status` rather than `trial`; it does not accept raw `custom`,
`entitlements`, `experiments`, or `instances`. See the
...
Playbook can silently over-grant, so construction refuses it rather than guessing.
</Aside>
...
ported and parity-locked, so a Rust gate and a TypeScript render agree given
the same user context — not only the same pre-resolved traits.
