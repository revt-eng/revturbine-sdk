...
- **`Playbook`** — the portable monetization config the SDK evaluates against: plans, entitlements, segments, and placements. In local mode you load it (typically `exported_config.json`) at init; in live mode the SDK fetches it.
...
| `Playbook` | `ExportedConfig`, `RevTurbineConfig` | the portable monetization config |
