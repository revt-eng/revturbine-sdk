...
<Aside type="tip">
In `local_only` mode the SDK evaluates all targeting rules, entitlements, and placement decisions client-side using the exported config — no network calls for decisioning, and clickstream events stay in `localStorage`. The one exception is a keyless, anonymous `sdk_init` adoption beacon fired once at startup (config-shape counts only, never user data); opt out with `anonymousTelemetry: false`, or set `previewMode: true` for a docs/example render.
</Aside>
