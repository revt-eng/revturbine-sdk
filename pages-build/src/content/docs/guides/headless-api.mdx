...
Headless is intended for **entitlement checks and user-context updates** —
`session.can()`, `session.update()`, usage reporting, event tracking. Two
rules follow from that:

1. **You supply the inputs.** Pass a `user` context at init, and in local mode
   pass the Playbook (`localRuntime: { playbook }`). There is no React
   provider doing this for you.
2. **Placements need an explicit slot.** To resolve a placement headlessly you
   provide a slot id **and** the surface template ids it accepts
   (`surfaceSlot: { id, surfaceTemplateIds }`). The React entry's automatic
   gate-placement resolution (`<Gate>`) is a React affordance — in headless
   code, denied-state upsell surfaces resolve through the slot config you
   declare.
...
// Local mode — no account, no network: user + Playbook are the inputs.
...
// Hosted mode — swap the local runtime for your tenant credentials:
// const session = await initRevTurbine({
//   tenantId: 'tenant_abc',
//   apiKey: 'rt_live_xxx',
...
// Entitlement — the primary headless verbs
const result = await session.can('brand_kit');
if (!result.allowed) {
  console.log('Denied:', result.reason);
}
session.update({ usage: { api_calls: 150 } });

// Placement — explicit slot id + the surface template ids it accepts
const banner = session.placement({
  surfaceSlot: { id: 'upsell_banner', surfaceTemplateIds: ['general_banner'] },
});
await banner.load();
if (banner.visible) {
...
// Entitlement gate with a denied-state surface
...
// update() patches any context field except identity `id`. Usage goes under
// the `usage` key — a bare handle is not a context field and is dropped with
// a dev warning.
session.update({ usage: { api_calls: 150 } });
session.update({ plan_handle: 'enterprise', plan: { handle: 'enterprise', name: 'Enterprise' } });
...
// Short-lived process (script, job, edge handler)? Flush before exit so
...
|---|---|---|---|
| `surfaceSlot` | `{id, name?, surfaceTemplateIds}` | — | Canonical surface slot (preferred). Headless resolution needs the slot id AND the surface template ids it accepts |
...
export function usePlacement(slotId: string) {
  ...
  onMounted(async () => {
    const session = await initRevTurbine(options); // your init options
...
Edit the code in any of these — all run in `local_only` mode with no backend.

<a id="headless-placement"></a>

**PlacementController** — driving a placement imperatively.

<SandpackPlayground scenarioId="headless-placement" client:only="react" />

<a id="headless-gate"></a>

**EntitlementGate** — an entitlement check without React.

<SandpackPlayground scenarioId="headless-gate" client:only="react" />

<a id="headless-session"></a>

**SdkSession one-shot** — a one-shot session evaluation.
