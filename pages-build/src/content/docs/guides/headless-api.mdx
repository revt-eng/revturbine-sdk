...
Runs in `local_only` mode with no backend. Two more headless scenarios are on the [Interactive Playground](/playground/#headless-api): [PlacementController](/playground/#headless-placement) and [SdkSession one-shot](/playground/#headless-session).
