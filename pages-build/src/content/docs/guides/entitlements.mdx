...
Entitlement checks are **fail-closed** — if a check can't produce an affirmative grant, `can` returns `allowed: false` rather than granting access. When the entitlement service is unreachable the reason is `'entitlement_service_unavailable'`, so you can still tell an outage apart from a real denial.

```ts
// If the entitlement service is unreachable:
// { status: 'denied', allowed: false, reason: 'entitlement_service_unavailable' }
```
