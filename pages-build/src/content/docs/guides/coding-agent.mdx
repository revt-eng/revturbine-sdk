...
2. Install @revturbine/sdk and wrap the app in <RevTurbineProvider> if it isn't
   already. Use local_only mode with a bundled exported_config.json fixture so it
   runs with no account.
3. Add a minimal exported_config.json fixture with: a free and a pro plan, one
...
revturbine verify ./exported_config.json

# 3. Preview / evaluate the candidate against real inputs.
revturbine preview ./exported_config.json
revturbine evaluate ./exported_config.json --handle data_export

# 4. Stage it through the review lifecycle (draft -> submit -> approve -> deploy).
revturbine deploy ./exported_config.json
```
