...
//    the same SDK, evaluating the same Playbook in-process.
...
<Gate id="export" can="data_export" onCtaClick={(path) => router.push(path.url ?? '/pricing')}>
  <button onClick={exportData}>Export</button>
</Gate>
```
...
In the **before**, changing the beta list or the upgrade copy is a code change + deploy + eng ticket, and your app has to *know about plans*. In the **after**, your app knows only **slots and entitlements** — a PM edits the targeting and copy in the dashboard, and nothing ships. The first monetization strategy is never the last; this is what keeps your code from changing every time it moves.
</Aside>
