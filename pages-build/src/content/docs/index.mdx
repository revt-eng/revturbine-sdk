...
<CardGrid>
  <Card title="Decisioning, not just rendering">
    RevTurbine decides — caps, cooldowns, eligibility, and priority across competing placements. One `<Slot>` (or `usePlacement()`) returns the winner. No client-side orchestration, no homegrown segment logic. ML-scored propensity is on the roadmap, not shipped — see [Beyond gating](/docs/guides/beyond-gating/).
  </Card>
