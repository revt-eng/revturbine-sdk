...
`npx revturbine create` scaffolds RevTurbine into your app: it detects your package manager and stack, installs `@revturbine/sdk`, pins `@revturbine/cli` into the repo, adds a starter Playbook that runs in local mode with no account, and installs the Agent Skills — then points you at the `revturbine-start-here` skill, which walks the rest. If you start this way, skip to step 3 and register an account whenever you want hosted features.

**Manual install** — two packages, both on the public npm registry; no auth token or registry config.
...
Create an account in the studio, then connect the CLI to it. Local mode stays free, and hosted is free through your first $20K/month in new revenue — see [pricing](https://revturbine.com/pricing).
...
1. **Wrap your app in `RevTurbineProvider`.** This example runs in `local_only` mode — no backend and no network calls for decisioning — from a `playbook.json` you ship with your app (see step 4 for where it comes from).
...
<CardGrid>
  <Card title="RevTurbine UI (studio)" icon="laptop">
    Edit visually at [**revturbine.com/app**](https://revturbine.com/app) — define plans, entitlements, targeting rules, and placements, preview their impact, and deploy. Export a `playbook.json` snapshot anytime for local mode.
  </Card>
...
```bash
revturbine verify ./playbook.json   # schema-validate locally, no writes
...
<Aside type="tip">
`revturbine export --save playbook.json` pulls your live config back down — handy for bundling into the SDK's `local_only` mode.
</Aside>
