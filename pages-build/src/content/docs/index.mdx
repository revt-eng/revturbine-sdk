...
3. **Gate a feature.** Wrap gated UI in `<Gate>`. It renders its children when the user is entitled, and automatically shows the configured upgrade placement on denial — no manual check.
    ...
    function BatchExport() {
      return (
        <Gate id="batch_export_gate" check={{ entitlement: 'batch_export' }}>
          <BatchExportButton />
        </Gate>
      );
    }
    ...
    For custom deny UI, use the `useEntitlement` hook and branch on the result:

    ```tsx
    import { useEntitlement } from '@revturbine/sdk';

    function BatchExport() {
      const { allowed, denied, result } = useEntitlement({ handle: 'batch_export' });
      if (denied) return <UpgradePrompt reason={result?.reason} />;
      return <BatchExportButton enabled={allowed} />;
    }
    ```

    For an imperative check (server-side or in an event handler), call `await rt.can('batch_export')`. See the [Entitlements guide](/guides/entitlements/).
